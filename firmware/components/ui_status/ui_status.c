...
static char s_idle_hint_text[UI_HINT_TEXT_MAX] = "Hold for voice";
static char s_device_name[16] = "XC";
...
static void render_scene_locked(ui_status_icon_scene_t scene, const char *status, const char *hint)
{
    ...
    lv_color_t bg = lv_color_hex(0x050a12);
    lv_color_t text = lv_color_hex(0xd7e4ed);
    lv_color_t muted = lv_color_hex(0x7194a8);
    lv_color_t hint_color = error ? lv_color_hex(0xff8a9b) : lv_color_hex(0x41d9ff);
...
static void create_status_ui(void)
{
    ...
    lv_obj_set_style_text_color(s_screen, lv_color_hex(0xd7e4ed), 0);
    ...
    lv_obj_set_style_text_color(s_top_label, lv_color_hex(0x7194a8), 0);
    ...
    lv_obj_set_style_text_color(s_status_label, lv_color_hex(0xd7e4ed), 0);
    ...
    lv_obj_set_style_text_color(s_hint_label, lv_color_hex(0x41d9ff), 0);
...
{
    strlcpy(s_device_name, device_name && device_name[0] ? device_name : "XC",
...
void ui_status_set_advertising(void)
{
    ...
    set_scene(UI_STATUS_ICON_PAIRING, "XC offline", "Open XC Buddy on Mac");
}
...
void ui_status_set_pairing(const char *device_name)
{
    ...
    set_scene(UI_STATUS_ICON_PAIRING, "XC offline", device_name ? device_name : "XC-0000");
}
...
{
    set_scene(UI_STATUS_ICON_TRANSCRIBING, "Confirm", "Front: send  Side: cancel");
}
